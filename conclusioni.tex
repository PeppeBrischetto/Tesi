...
Le simulazioni numeriche costituiscono oggi un potente strumento per studiare la risposta di un sistema di rivelazione e per dedurre utili informazioni sulle caratteristiche tecniche ottimali.
...
Nell'ambito del progetto NUMEN è stato sviluppato e implementato, per la prima volta, un tool di simulazioni capace di descrivere in modo completo la cinematica di reazione, il moto degli eiettili attraverso gli elementi magnetici dello spettrometro MAGNEX e la loro interazione con un muro di telescopi $\Delta E - E$ a stato solido.
Tale tool si compone essenzialmente di due parti: la prima calcola la cinematica e risolve le equazioni del moto degli eiettili fino al decimo ordine, la seconda si occupa di 
%Per poter riflettere il comportamento reale degli ioni nello spettrometro, il tool deve incorporare le correlazioni dovute sia alla cinematica sia alle proprietà ottiche dello spettrometro.
