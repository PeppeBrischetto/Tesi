...
Come primo esempio, si è scelto di illustrare i risultati di un caso ideale in cui le particelle primarie hanno un'energia di 1000~MeV ed incidono ortogonalmente e centralmente al telescopio.
...
\begin{figure} [!t]
	\centering
	\includegraphics[width=\textwidth, keepaspectratio]{Grafici_Tesi/deltaE_ETot_punti_grandi.eps}
	\caption{Matrice.} \label{fig:deltaE_ETot}
\end{figure}
...
\begin{figure} [!p]
	\centering
	\includegraphics[scale=0.6]{Grafici_Tesi/ETot.eps}
	\caption{Spettro.} \label{fig:ETot}
\end{figure}


\begin{figure} [!p]
	\centering
	\includegraphics[scale=0.6]{Grafici_Tesi/deltaE_Tot.eps}
	\caption{Spettro .} \label{fig:deltaE_Tot}
\end{figure}
...
Inoltre, anche in questo caso viene utilizzato il modello dei processi fisici dell'esempio precedente.
Le corrispondenti matrici $\Delta E_{SiC} - E_{tot}$ sono rappresentate in Figura~\ref{fig:deltaE_ETot_full_energy}: in primo luogo, si vede la formazione delle bande previste dalla formula di Bethe-Block.
Inoltre, a differenza del caso precedente si nota la comparsa di eventi in cui la perdita di energia $\Delta E_{SiC}$ viene misurata correttamente, mentre l'energia $E_{tot}$ ha un valore inferiore rispetto a quello ``corretto''. Ciò dipende da quegli eventi in cui le particelle escono fuori dal rivelatore al CsI, non rilasciando totalmente la loro energia residua.
A causa del motivo precedentemente citato, anche questi eventi introducono una componente di errore nella PID.
Per meglio visualizzare la frazione di eventi spuri (\textcolor{red}{si possono definire così?}), si è riportata in Figura~ la proiezione sull'asse verticale della ``fetta'' di energia compresa fra 600 e 700~MeV: come si può notare, tali eventi costituiscono un continuo che si estende fino ad energie prossime a zero. 
