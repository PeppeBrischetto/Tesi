...
\subsection{\iflanguage{italian}{Modello con interazioni adroniche}{Model with adronic interactions}} \label{par:interazioni_adroniche}
...
Dunque, è evidente che la maggior parte degli eventi spuri è originata da raccolta di carica incompleta, come d'altronde era lecito aspettarsi vista la bassa probabilità che avvenga un'interazione adronica.
...
\begin{figure}[!p] 
	...
	\caption{La percentuale di nuclei potenzialmente identificati in modo erroneo in funzione di $E_{CsI}$, considerando una superficie totale di $1.5 \times 1.5$~cm\ap{2}: in~(a) la contaminazione da specie atomiche con $Z$ maggiore verso quelle con $Z$ minore, in (b) il viceversa.} \label{fig:leakage_res_1.5}
\end{figure}
...
\begin{figure}[!p] 
	...
	\caption{La percentuale di nuclei potenzialmente identificati in modo erroneo in funzione di $E_{CsI}$, considerando una superficie totale di $2 \times 2$~cm\ap{2}: in~(a) la contaminazione da specie atomiche con $Z$ maggiore verso quelle con $Z$ minore, in (b) il viceversa.} \label{fig:leakage_res_2}
\end{figure}
...
In accordo con quanto trovato, si può affermare che, al fine di minimizzare gli eventi con raccolta di carica incompleta, è opportuno avere la maggiore superficie di rivelazione possibile, come si evince dalla Figura~.
...
Tuttavia, avere maggiori superfici significa anche aumentare il verificarsi del pile-up, in quanto cresce la probabilità che due ioni vadano a finire nello stesso telescopio, producendo un segnale somma di due segnali.
Dal momento che per gli obiettivi di NUMEN è necessario lavorare con fasci di alta intensità, è fondamentale che la probabilità di pile-up sia piccola.

Quindi, due aspetti diversi spingono in direzioni opposte: di conseguenza, è necessario cercare una situazione di compromesso fra le due esigenze.
...
Il primo rappresenta un traguardo concreto nelle 
...
%\clearpage
\subsection{\iflanguage{italian}{Studio del substrato morto}{Study of dead-substrate}}
