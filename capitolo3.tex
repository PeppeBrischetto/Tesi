...
Scegliendo opportuni valori per $a$ e $b$, si è assunta per il rivelatore al SiC una risoluzione energetica dello 0.8\% FWHM a 19.4~MeV, mentre per il rivelatore allo CsI dell'1.8\% FWHM a 850~MeV, in affinità con i valori tipici di questi dispositivi. (\textcolor{red}{Devo specificare i valori di a e b scelti nei due casi?})
...
È bene ricordare che tale tecnica prevede la correlazione fra la perdita di energia $\Delta E$ e l'energia cinetica totale $E_{tot}$. In riferimento al caso in esame, $\Delta E$ corrisponde all'energia persa dalla particella primaria nel rivelatore al~SiC (che da ora in poi indicheremo con $ \Delta E_{SiC}$), mentre $E_{tot}$ equivale alla sua energia cinetica iniziale. Nelle simulazioni svolte, quest'ultima quantità è data da
\begin{equation}
	E_{tot} \, = \, \Delta E_{SiC} + E_{sub} + E_{CsI}
\end{equation}
...
Dal momento che uno degli aspetti studiati in questo lavoro consiste nel valutare se la correlazione $\Delta E_{SiC} - E_{CsI}$ può garantire prestazioni analoghe a quella $\Delta E_{SiC} - E_{tot}$, nel prosieguo verrà specificato quale tipo di correlazione si sta considerando.
...
Come primo esempio, si è scelto di illustrare i risultati di un caso ideale in cui le particelle primarie sono monoenergetiche ed incidono ortogonalmente e centralmente al telescopio.
...
Questa affermazione viene confermata dalla Figura~, in cui è possibile notare che i luoghi di \ce{^{18}O}, \ce{^{18}F} e \ce{^{18}Ne} sono chiaramente distinti.
Al di sotto di ciascuna delle tre matrici sono presenti degli eventi in cui l'energia residua è stata misurata correttamente, mentre la perdita di energia risulta inferiore al valore corretto: si tratta di eventi in cui la carica prodotta nel rivelatore al SiC è stata raccolta parzialmente. 

In Figura~ e in Figura~ sono riportate, rispettivamente, le proiezioni sull'asse $E_{tot}$ e sull'asse $\Delta E_{SiC}$ delle matrici $\Delta E_{SiC} - E_{tot}$: esse rappresentano, dunque, gli spettri energetici misurati dal rivelatore al SiC e dal rivelatore allo CsI.


%\subsection{\iflanguage{italian}{Processi fisici senza interazioni adroniche}{Physics process without adronic interactions}}
\subsection{\iflanguage{italian}{Energia }{Physics process without adronic interactions}}
