...
È bene ricordare che tale tecnica prevede la correlazione fra la perdita di energia $\Delta E$ e l'energia cinetica totale $E_{tot}$. In riferimento al caso in esame, $\Delta E$ corrisponde all'energia persa dalla particella primaria nel rivelatore al~SiC, mentre $E_{tot}$ equivale alla sua energia cinetica iniziale. Nelle simulazioni svolte, quest'ultima quantità è data da
\begin{equation}
	E_{tot} \, = \, \Delta E + E_{sub} + E_{resid}
\end{equation}
laddove $E_{sub}$ è l'energia persa nel substrato morto ed $E_{resid}$ è l'energia residua persa nel cristallo allo CsI.
...
Dal momento che uno degli aspetti studiati in questo lavoro consiste nel valutare se la correlazione $\Delta E - E_{resid}$ può garantire prestazioni analoghe a quella $\Delta E - E_{tot}$, per maggiore chiarezza nel prosieguo verrà specificato quale tipo di correlazione si sta considerando.
