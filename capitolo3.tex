...
È bene precisare che, per l'\ce{^{19}O}, il valore indicato non deriva da misure sperimentali ma considerazioni sull'ordine del processo stesso.
\begin{table} [p!]
	\begin{center}
		\renewcommand{\arraystretch}{1.2}
...
L'efficienza di rivelazione del segnale resta del $(90.4 \pm 0.2)$~\%, mentre il rapporto S/B risulta essere pari a $10939 \pm 38 $, inferiore a quello trovato con la correlazione $\Delta E_{SiC} - E_{meas}$.
...
\begin{table} [t!]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{La contaminazione dei diversi ioni nella ROI dell'\ce{^{20}O^{8+}} individuata dal taglio grafico in Figura~\ref{fig:deltaE_Emeas_taglio}.} \label{tab:contaminazione_deltaE_Emeas_1.5per1.5a}
\end{table}
...
%%Moltiplicando i numeri della terza colonna con quelli della quarta, si può determinare la probabilità di ottenere, a seguito di una reazione nucleare, uno ione in un determinato stato di carica.
%questo è il fattore con cui bisogna riscalare le percentuali precedentemente mostrate per stimare le contaminazioni reali.
Analogamente a quanto visto in precedenza, sono stati ricavati i fattori di scala, i quali sono stati moltiplicati ai valori della Tabella~\ref{tab:sez_d'urtoa} per ottenere le contaminazioni reali.
...
Sovrapposti a tale matrice, si possono notare gli eventi degradati dell\ce{^{20}O^{8+}} che contaminano il segnale: purtroppo la rappresentazione $x_{foc} - E_{CsI}$ non risulta utile, in questo caso, a separare lo ione di interesse dai contaminanti, in quanto questi hanno un'energia $E_{CsI}$ uguale a quella del segnale.
\begin{figure} [!p]
	\centering
	\includegraphics[width=\textwidth, keepaspectratio]{Grafici_Tesi2/PIDnew/xf_csi_taglio_quadrato2.png}
...
\begin{figure} [!p]
	\centering
	\includegraphics[width=\textwidth, keepaspectratio]{Grafici_Tesi2/PIDnew/tof_csi_taglio_quadrato.png}
...
\begin{figure} [!p]
	...
	\caption{La matrice $x_{foc} - E_{CsI}$ per gli eventi selezionati dal taglio grafico in Figura~\ref{fig:deltaE_Ecsia}.} \label{fig:xf_Ecsi_O20a}
\end{figure} 
\begin{figure} [!p]
	\centering
	\includegraphics[width=\textwidth, keepaspectratio]{Grafici_Tesi2/PID/tof_csi_taglio.png}
	\caption{La matrice $\mbox{tof} - E_{CsI}$ per gli eventi selezionati dal taglio grafico in Figura~\ref{fig:deltaE_Ecsi}.} \label{fig:tof_Ecsi_O20a}
\end{figure} 
...
\begin{table} [t!]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Utilizzando la correlazione $\Delta E_{SiC} - E_{CsI}$, per ogni ione si riporta il numero di eventi nella ROI normalizzato ad un evento di segnale.} \label{tab:contaminazioni_deltaE_Ecsi_riscalate_1per1}
\end{table}
...
\begin{table} [t!]
	\begin{center}
		...
		\end{tabular}
	\end{center}
	\caption{Utilizzando la correlazione $\Delta E_{SiC} - E_{CsI}$, per ogni ione si riporta il numero di eventi nella ROI normalizzato ad un evento di segnale.} \label{tab:contaminazioni_deltaE_Ecsi_riscalate_2per2}
\end{table}
