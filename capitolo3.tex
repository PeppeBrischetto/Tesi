...
Considerando un sistema di riferimento in cui l'asse $z$ è individuato dalla direzione di propagazione delle particelle e l'asse $y$ è verticale, i telescopi sono fra loro separati di 2~mm lungo la direzione $x$ e di 1~mm lungo la direzione~$y$.
...
\subsection{\iflanguage{italian}{Elaborazione post-simulazione}{Post-simulation processing}} \label{par:post-processing}
...
\begin{figure} [!p]
	...
	\caption{L'andamento della funzione di risposta $f(x)$ assunta per descrivere gli effetti di bordo del rivelatore al SiC nel caso di dimensioni trasversali di 1~cm $\times$ 1~cm.} \label{fig:funzione_risposta}
\end{figure}
...
La simulazione è stata svolta considerando un proiettile di \ce{^{20}Ne} incidente su un bersaglio di \ce{^{76}Ge} a 700~MeV di energia.
...
Gli eiettili simulati sono \ce{^{20}O^{8+}}, \ce{^{21}O^{8+}}, \ce{^{20}F^{8+}}, \ce{^{21}F^{8+}}, \ce{^{20}Ne^{8+}}, \ce{^{21}Ne^{8+}} e \ce{^{22}Ne^{8+}}: mentre il primo rappresenta lo ione di interesse, gli altri sono le principali sorgenti di contaminazione quando si studia una reazione di DCE in cui l'\ce{^{20}O^{8+}} ed il residuo della reazione (\ce{^{76}Se}) sono nel loro stato fondamentale.
Altri ioni, come ad esempio il \ce{^{19}F^{8+}} e l'\ce{^{19}O^{8+}}, e altri stati di carica, per esempio i $9^+$ e i $10^+$, non sono stati presi in considerazione perché, per ragioni cinematiche e per le proprietà ottiche dello spettrometro, vengono rivelati in regioni del FPD lontane da quella in cui arriva lo ione di interesse o sono addirittura fuori dall'accettanza del FPD.
Per ciascuno ione simulato è stato generato un milione di eventi.
Il telescopio ha dimensioni trasversali di 1.5~cm $\times$ 1.5~cm ed è posto a 15~cm di distanza dal piano in cui vengono prodotte le particelle primarie, che coincide con il piano di ingresso nel FDP.
Nella simulazione si è assunto che l'asse ottico dello spettrometro fosse posto a 3\textdegree{} nel sistema di riferimento del laboratorio.
...
La rigidità magnetica di riferimento è stata scelta in modo che la traiettoria corrispondente alla transizione in cui l'\ce{^{20}O^{8+}} e il residuo di \ce{^{76}Se} sono nel loro stato fondamentale fosse focalizzata al centro del FPD, ovvero in corrispondenza del telescopio.
...
Come si può notare, per ogni ione il range energetico esplorato dal telescopio è di circa 9~MeV, in buon accordo con la~\ref{eq:finestra_Brho}.
...
Per dare una stima quantitativa della percentuale degli eventi degradati, che potrebbero costituire un fondo nella regione di interesse (Region Of Interest, ROI), si è proceduto nel modo seguente: supponendo di voler analizzare una reazione di DCE in cui l'\ce{^{20}O^{8+}} e il residuo di \ce{^{76}Se} sono nel loro stato fondamentale, si è effettuato nel plot $\Delta E_{SiC} - E_{meas}$ un taglio grafico attorno al luogo pertinente allo ione \ce{^{20}O^{8+}}, come mostrato in Figura~\ref{fig:deltaE_Emeas_taglio}.
\begin{figure} [!p]
	\centering
...
Il risultato di tale operazione, nel caso del taglio grafico considerato in Figura~\ref{fig:deltaE_Emeas_taglio}, è riportato in Tabella~\ref{tab:contaminazione_deltaE_Emeas_1.5per1.5}: come si può notare, le percentuali sono molto basse, a dimostrazione delle buone capacità di PID del telescopio~SiC-CsI.
...
Nella Tabella~\ref{tab:sez_d'urto} sono riportati i valori tipici delle sezioni d'urto sperimentali dei processi coinvolti in questa simulazione, normalizzati alla sezione d'urto sperimentale peculiare del DCE.
\begin{table} [p!]
	\begin{center}
		\renewcommand{\arraystretch}{1.2}
		\begin{tabular} {cccccccl}
			...
			\hline
			$5^{\mbox{th}}$ order multi-   & & \ce{^{21}O}  & &        0.1       & & & $8^+ \sim 0.99963$ \\
			nucleon transfer              & &              & &                 & & &  \\
	...
	\end{center}
	\caption{I valori tipici delle sezioni d'urto sperimentali di diversi processi di reazioni nucleari normalizzati alla sezione d'urto del DCE. Tali valori sono stati ricavati dalla collaborazione NUMEN (si vedano, ad esempio, \cite{calabrese:phd19_2} e~\cite{spatafora:prc19}). I valori sono stati arrotondati verso l'alto per ottenere delle stime prudenti per le contaminazioni reali. Si riporta, inoltre, la probabilità di avere uno ione in un determinato stato di carica dopo aver attraversato un foglio di carbonio~\cite{shima:adndt92}. Si sottolinea che, nel caso dell'\ce{^{21}O}, il valore della sezione d'urto indicato non deriva da misure sperimentali ma da considerazioni sull'ordine del processo stesso in relazione ad altri processi di trasferimento di nucleoni di cui è nota la sezione d'urto.} \label{tab:sez_d'urto}
\end{table}
Tali valori, arrotondati verso l'alto per ottenere delle stime prudenti per le contaminazioni reali, sono stati ricavati da misure sperimentali svolte dalla collaborazione NUMEN (si vedano, ad esempio, \cite{calabrese:phd19_2} e~\cite{spatafora:prc19}).
%Essi sono stati prudenzialmente arrotondati verso l'alto per
Nella stessa tabella viene, inoltre, riportata la probabilità di avere uno ione in un certo stato di carica dopo il passaggio attraverso un foglio di carbonio~\cite{shima:adndt92}. 
È bene precisare che, per l'\ce{^{21}O}, il valore indicato non deriva da misure sperimentali ma da considerazioni sull'ordine del processo stesso in relazione ad altri processi di trasferimento di nucleoni di cui è nota la sezione d'urto.
...
Dividendo il numero di segnali per la somma degli eventi di fondo, si ottiene una quantità nota come \emph{rapporto segnale-background}~(S/B), che in questo caso risulta essere di $24200 \pm 60$.
A partire da questo valore è possibile risalire alla sensibilità di misura nel modo seguente: sapendo che la sezione d'urto tipica del DCE nel range di energia di eccitazione studiato è di circa 1000~nb (si veda \cite{calabrese:phd19_2}), dal rapporto S/B si deduce che il fondo corrisponde a circa 40~pb ad 1$\sigma$ di errore; dunque, considerando 5$\sigma$, si trova una sensibilità di circa 200~pb.
...
Pertanto, con la sensibilità di misura stimata, l'apparato sperimentale sarebbe in grado di misurare la reazione di DCE nella regione di interesse.
...
L'efficienza di rivelazione del segnale resta del $(90.4 \pm 0.2)$~\%, mentre il rapporto S/B risulta essere pari a $10940 \pm 30 $, inferiore a quello trovato con la correlazione $\Delta E_{SiC} - E_{meas}$.
...
Poiché la correlazione $\Delta E_{SiC} - E_{CsI}$ ha dimostrato offrire una sensibilità di misura adatta agli scopi del progetto, da questo momento in poi verranno presentate soltanto le matrici relative a questa correlazione.
...
Questo studio potrebbe infatti essere interessante per la ricerca delle risonanze giganti doppie di Gamow-Teller, mai osservate, ma importanti per la loro connessione con il \doppiobeta~\cite{shimizu:prl17}.
...
Nella Tabella~\ref{tab:sez_d'urtoa} sono riportati i valori tipici delle sezioni d'urto dei processi coinvolti (si vedano \cite{calabrese:phd19_2} e \cite{spatafora:prc19}), espressi relativamente alla sezione d'urto del DCE, e le probabilità dei diversi stati di carica~\cite{shima:adndt92}.
\begin{table} [p!]
	\begin{center}
		\renewcommand{\arraystretch}{1.2}
		\begin{tabular} {cccccccl}
		...
		\end{tabular}
	\end{center}
	\caption{I valori tipici delle sezioni d'urto sperimentali di diversi processi di reazioni nucleari normalizzati alla sezione d'urto del DCE. Tali valori sono stati ricavati dalla collaborazione NUMEN (si vedano, ad esempio, \cite{calabrese:phd19_2} e~\cite{spatafora:prc19}). I valori sono stati arrotondati verso l'alto per ottenere delle stime prudenti per le contaminazioni reali. Si riporta, inoltre, la probabilità di avere uno ione in un determinato stato di carica dopo aver attraversato un foglio di carbonio~\cite{shima:adndt92}. Si sottolinea che, nel caso dell'\ce{^{19}O}, il valore della sezione d'urto indicato non deriva da misure sperimentali ma da considerazioni sull'ordine del processo stesso in relazione ad altri processi di trasferimento di nucleoni di cui è nota la sezione d'urto.} \label{tab:sez_d'urtoa}
\end{table}
%Moltiplicando il valore della sezione d'urto sperimentale tipica di un processo per la probabilità di ottenere uno stato di carica, si può determinare la probabilità di produrre, a seguito di una reazione nucleare, uno ione in un determinato stato di carica;
%%Moltiplicando i numeri della terza colonna con quelli della quarta, si può determinare la probabilità di ottenere, a seguito di una reazione nucleare, uno ione in un determinato stato di carica.
%questo è il fattore con cui bisogna riscalare le percentuali precedentemente mostrate per stimare le contaminazioni reali.
Analogamente a quanto visto in precedenza, sono stati ricavati i fattori di scala, i quali sono stati moltiplicati con i valori della Tabella~\ref{tab:contaminazione_deltaE_Emeas_1.5per1.5a} per ottenere le contaminazioni reali.
\begin{table} [h!]
	\begin{center}
		\renewcommand{\arraystretch}{1.2}
		\begin{tabular} {cccc}
...
Dunque, se si vuole effettuare uno studio delle reazioni di DCE ad elevate energie di eccitazione è necessario affiancare alla tecnica $\Delta E - E$ correlazioni che includono altri parametri, in grado di separare il segnale dai contaminanti.
...
%La base di volo di uno ione all'interno di MAGNEX viene determinata attraverso la ricostruzione della traiettoria 
In uno spettrometro magnetico la base di volo non è la stessa per tutti gli ioni ma cambia da evento a evento a seconda del rapporto massa-carica, dell'energia e dell'angolo di ingresso.
Nel caso di MAGNEX essa viene determinata attraverso i software di trasporto ottico, i quali ricostruiscono la traiettoria di ogni ione rivelato.
...
Inoltre, in Figura~\ref{fig:xf_Ecsi} è evidente la presenza di eventi degradati, che anche in questo caso possono andare a posizionarsi sui luoghi caratteristici di altri ioni.
...
Al fine di separare il segnale dal fondo si può, allora, utilizzare la correlazione $\mbox{tof} - E_{CsI}$: in Figura~\ref{fig:tof_Ecsi_O20a}, dove sono riportati gli eventi selezionati dal taglio grafico in Figura~\ref{fig:deltaE_Ecsi_altaE}, è possibile notare come questa rappresentazione permetta di distinguere chiaramente il segnale dagli ioni F e Ne.
...
Tuttavia, la Figura~\ref{fig:tof_Ecsi_O20a} può fornire un'indicazione della precisione minima necessaria per riuscire a separare lo ione di interesse dal fondo: dalle simulazioni si può infatti dedurre che essa deve essere inferiore a 10~ns, altrimenti gli eventi originati dal F potrebbero sovrapporsi a quelli dell'\ce{^{20}O^{8+}}.
...
Alla luce delle considerazioni effettuate, risulta che una misura del tempo di volo con precisione migliore di 10~ns è fondamentale per poter analizzare le reazioni di DCE ad elevata energia di eccitazione.
...
In uno spettrometro dispersivo la riduzione della dimensione orizzontale del telescopio posto su piano focale porta alla riduzione del range di energie cinetiche accettato: in effetti, confrontando la Figura~\ref{fig:xf_ecsi_1per1} e la Figura~\ref{fig:xf_Ecsi} si può notare che mentre nella prima l'intervallo di~$x_{foc}$ visto dal telescopio è di circa 2.3~cm, nella seconda tale intervallo è di 2.8~cm. Poiché negli spettrometri magnetici vi è una relazione di proporzionalità tra la posizione degli eiettili al piano focale e la loro energia cinetica, se ne deduce che nel primo caso l'intervallo di energia cinetica visto dal telescopio è più piccolo.
...
Calcolando il rapporto S/B si trova un valore di $4880 \pm 20$, minore di quello trovato nel caso precedente.
...
Anche il rapporto S/B risulta maggiore, con un valore di $16530 \pm 30$.
...
\end{equation}
Poiché questi fenomeni sono governati dalla statistica di Poisson, la precedente espressione assume la forma
