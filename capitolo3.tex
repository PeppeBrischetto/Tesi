...
Le percentuali di contaminazione, riscalate per la sezione d'urto e per la probabilità di ottenere un certo stato di carica, sono mostrate in Tabella~\ref{tab:contaminazioni_deltaE_Ecsi_riscalate}: confrontando quest'ultima con la Tabella~\ref{tab:contaminazioni_riscalate} si può osservare che le percentuali sono, per tutti gli ioni considerati, molto simili.
Il rapporto S/B risulta essere di $0.021 \pm 0.001 $, dunque migliore di quello trovato con la correlazione $\Delta E_{SiC} - E_{meas}$.
Sulla base di questo risultato è possibile sostenere che non è necessario effettuare una calibrazione energetica dei telescopi, in quanto ai fini della PID non vi è l'esigenza di sommare i valori di energia misurati dai due rivelatori.
...
Nei paragrafi precedenti si è evidenziato come i telescopi SiC-CsI riescano a produrre nelle correlazioni $\Delta E - E$ un'efficace separazione degli ioni sia in $Z$ sia in $A$; tuttavia, lo spettrometro offre anche altre correlazioni che potrebbero migliorare notevolmente le capacità di PID. 
...
Inoltre, nel paragrafo precedente abbiamo visto che, se nelle correlazioni si utilizza $E_{CsI}$ invece di $E_{meas}$, le capacità di PID migliorano.
Per questo motivo, nel prosieguo verranno trattate soltanto le correlazioni che coinvolgono l'energia $E_{CsI}$ misurata dal cristallo allo~CsI piuttosto che l'energia cinetica~$E$.
