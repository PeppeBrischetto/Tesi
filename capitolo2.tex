...
\section{\iflanguage{italian}{I telescopi SiC-CsI}{SiC-CsI telescope detectors}} \label{sez:telescopi_sic_csi}
...
Oltre alla resistenza alle radiazioni, i rivelatori al SiC presentano un ulteriore vantaggio rispetto a quelli al silicio: la maggiore larghezza della gap determina una riduzione del numero di coppie generate per agitazione termica, che risulta in una diminuzione della corrente inversa e del livello di rumore.
...
\subsection{\iflanguage{italian}{I telescopi utilizzati nel test}{Telescope detectors used in the test}} \label{par:telescopi}

\begin{figure} [!p]
	\centering
	\includegraphics[width=\textwidth, keepaspectratio]{Grafici/sic_etichette.png}
	\caption{I rivelatori al carburo di silicio (SiC) utilizzati nel test: a sinistra il SiC~B, a destra il SiC~A.} \label{fig:sic}
\end{figure}

I due prototipi di rivelatori al SiC utilizzati nel test sono mostrati in Figura~\ref{fig:sic}: come è possibile notare, essi differiscono innanzitutto per la segmentazione dell'area attiva, in quanto uno (SiC~A) è costituito da un'unica pad, mentre l'altro (SiC~B) è suddiviso in quattro regioni. 
...
Essi hanno dimensioni differenti: uno (CsI~A) è da $1 \times 1$~cm\ap{2}, l'altro (CsI~B) è suddiviso in quattro regioni da $1.5 \times 1.5$~cm\ap{2}. 
...
\begin{figure} [!t]
	...
	\caption{I due telescopi SiC-CsI utilizzati nel test: a sinistra il Tel~A, a destra il Tel~B.} \label{fig:telescopi}
\end{figure}
...
Il SiC~A e il SiC~B sono stati assemblati, rispettivamente, con il CsI~A e il CsI~B, così da formare i due telescopi (Tel~A e Tel~B) mostrati in Figura~\ref{fig:telescopi}.
...
\paragraph{Elettronica standard} \label{par:elettr_standard}
...
\paragraph{Elettronica VMM3a} \label{par:elettr_vmm3a}
