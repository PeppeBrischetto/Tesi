...
L'ingresso nel volume attivo è definito da una finestra di Mylar spessa 1.5~$\mu$m, la quale è utilizzata per contenere il gas, solitamente costituito da N35 isobutano (C\ped{4}H\ped{10}) a pressioni di poche decine di~mbar.
...
Tale proprietà può essere spiegata nel modo seguente: quando una particella carica attraversa un rivelatore, oltre a ionizzare gli atomi del mezzo, può causare la comparsa nel reticolo cristallino di interstizi, vacanze o strutture più complesse.
...
Nella rivelazione di ioni pesanti ad energie maggiori di diverse decine di~MeV, i rivelatori al SiC esibiscono risoluzioni energetiche confrontabili con quelle dei dispositivi al silicio, in quanto, essendo rilasciata molta energia nel rivelatore, viene prodotto un numero elevato di portatori di carica.
...
Il risultato, riportato in Figura~, dimostra che col rivelatore al SiC è possibile ottenere una risoluzione energetica dello 0.4\%~FWHM per le particelle $\alpha$ dell'\ce{^{241}Am} a 5486~keV, mentre col rivelatore al silicio per le stesse particelle si è raggiunto un valore dello 0.22\%~FWHM.
In un altro test~ref, il rivelatore al SiC è stato adoperato per la realizzazione di un telescopio SiC-CsI, simile a quelli previsti per NUMEN.
...
Le correlazioni $\Delta E - E_{resid}$ sono riportate in Figura~\ref{fig:ciampi_deltaE_E}: come si può notare, i differenti elementi sono ben identificati fino a $Z \sim 20$, consentendo perfino un'identificazione in $A$ per $Z \sim 3$.


\begin{figure} [!t]
	\centering
	\includegraphics[width=\textwidth, keepaspectratio]{Grafici/ciampi-telescopio.png}
	\caption{Schema del muro di telescopi previsto per NUMEN.} \label{fig:ciampi_telescopio}
\end{figure}
...
\begin{figure} [!p]
	...
	\caption{Schema del muro di telescopi previsto per NUMEN.} \label{fig:ciampi_deltaE_E}
\end{figure}
...
Dalla ricerca è emerso che diversi studi sono stati condotti su tale argomento, i quali affermano concordemente che la robustezza del SiC è, a parità di condizioni, molto maggiore di quella del silicio~\cite{raciti:npa10, garcialopez:nimb16, nava:nima03}
